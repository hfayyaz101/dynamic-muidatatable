...
import { getListData } from "redux-axios-jwt/actions/apiActions";
...
export default function DunamicMUITable({
  ...
  useEffect(() => {
    ...
    dispatch(
      getListData(
        urlLink +
          "/?" +
          params.map((item) => item.name + "=" + item.value + "&").join(""),
        id_token,
        false
      )
    ).then((res) => {
  ...
  const mui_table_action = (page, table_state) => {
    let sort_string = "";
    let sort_array = table_state.columns.filter(
      (item) => item.sortDirection === "desc" || item.sortDirection === "asc"
    );
    ...
    let custom_filter_params = filter_list.map((fl) => {
      let temp_list_filter =
        table_state.filterList[
          table_state.columns.findIndex((x) => x.name === fl.name)
        ];

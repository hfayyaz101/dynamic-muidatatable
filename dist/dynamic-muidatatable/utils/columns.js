...
import { Cancel as CancelIcon, CheckCircle as CheckIcon } from "@material-ui/icons";
...
const displaySetter = (col, columnsCustom) => {
  ...
  if (custom_option.length !== 0 && Object.keys(custom_option[0]).filter(key => key === "display").length !== 0) {
...
export function generate_columns(data, columnsCustom) {
  ...
  let new_custom_column = columnsCustom.filter(op => op.name === "new_column_array");
  let new_colum = [];
